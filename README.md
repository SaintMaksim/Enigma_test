...
|------|-------------|---------|------------------|
|Ограниченное время хакатона| Высокая |Критическое|Фокус на MVP: почта → ML → таблица → ответ. Использовать Django Admin вместо кастомного UI на старте
|Интеграция ML-модели RoBERTa|Средняя|Высокое|Использовать готовые пайплайны HuggingFace. Создать заглушку для тестирования UI до готовности ML
|Работа с почтовым сервером (IMAP/SMTP)|Средняя|Среднее|Начать с mock-данных. Реальная интеграция на втором этапе хакатона
|Ошибки в настройке Docker Compose|Низкая|Среднее|Протестировать локально до хакатона. Подготовить рабочий шаблон docker-compose.yml
|Конфликты в Git при работе 5 человек|Средняя|Низкое|Чёткое разделение папок (backend/frontend). Использовать feature-branches + Pull Requests
|Галлюцинации или ошибки классификации ML|Средняя|Среднее|Валидация оператором перед отправкой. Логирование всех AI-предложений для анализа
|Несоответствие полей модели Mail и фронтенда|Высокая|Среднее|Согласовать контракт API заранее. Использовать TypeScript-интерфейсы или Swagger
|Проблемы с подключением PostgreSQL|Средняя|Высокое|Иметь fallback на SQLite для разработки. Проверить переменные окружения в .env
|Недостаточная производительность при большом объёме писем|Низкая|Среднее|Добавить пагинацию на фронтенде. Использовать индексы в БД. Кэширование частых запросов
|Отсутствие документации по API|Средняя|Низкое|Вести README.md в процессе разработки. Использовать Django REST Framework с авто-документацией
...
```bash
# Запуск только бэкенда и базы данных
docker-compose up --build -d

# Запуск всего стека (бэкенд + база данных + фронтенд)
